...
var calcul = function (expression) {
    ...
    var nbOuvrantes = expression.match(/\(/g);
    var nbFermantes = expression.match(/\)/g);
    // On vérifie si l'expression est valide
    if (!validRegexp.test(expression)) {
        util.log("Expression invalide : '" + expression + "'");
    }
    // On vérifie si le nombre de parenthèses est cohérent
    else if (nbOuvrantes && nbFermantes && nbOuvrantes.length != nbFermantes.length) {
        util.log("Gestion incohérente des parenthèses : " + nbOuvrantes.length + " ouvrantes, " +nbFermantes.length + " fermantes");
    }
    // TODO : Mettre en place une meilleure regexp pour valider l'expression (gestion des opérateurs en début/fin de chaine, opérations binaires/unaires, ...)
        ...
        while (elements.length > 0) {
            ...
            if (numRegexp.test(elem)) {
                valStack.push(elem);
//            util.log("On a poussé " + elem + " dans la pile valStack");
//            util.log(" => " + JSON.stringify(valStack));
...
//                        util.log("Parenthèse fermante, on calcul la sous expression!");
                            compute();
                        }
...
//                        util.log("Element prioritaire, on calcul la sous expression (" + elem + " VS " + opStack[opStack.length - 1] + ")");
                            opStack.push(elem);
                        }
                        else {
                            while (opStack.length > 0 && isPrioritaire(opStack[opStack.length - 1], elem)) {
//                            util.log("(" + opStack.length + ") Dernier opérateur prioritaire, on calcul la sous expression (" + opStack[opStack.length - 1] + " VS " + elem + ")");
                                compute();
                            }
                            opStack.push(elem);
                        }
                        break;
                    default:
                        util.log("Erreur : élément non supporté : '" + elem + "'");
                        break;
                }
//            if (elem != ')') {
//                util.log("On a poussé " + elem + " dans la pile opStack");
//                util.log(" => " + JSON.stringify(opStack));
//            }
            }
        }
...
//    util.log(operande1 + ' ' + operateur + ' ' + operande2 + " = " + resultat);
};
